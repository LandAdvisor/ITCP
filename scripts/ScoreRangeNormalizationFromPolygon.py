...
NewMin = sys.argv[3]
NewMax = sys.argv[4]
OutputRaster = sys.argv[5]
TempRaster = "%scratchWorkspace%\\temprst1015.tif"

# Add field to hold normalized value...
arcpy.AddField_management(InputPolygonFeatureClass, "norm", "FLOAT")

# Determine max and min in field...
OldMax = 0.0
OldMin = 999999999999.0
with arcpy.da.SearchCursor(InputPolygonFeatureClass, [FieldToNormalize]) as FeatureClassCursor:
    for FeatureClassRow in FeatureClassCursor:
        ...
        if FieldValue is not None:
            if FieldValue > OldMax:
                OldMax = FieldValue
            if FieldValue < OldMin:
                OldMin = FieldValue
...
if OldMin == OldMax:
    ...
    Expr = str(NewMin) + " + ((" + str(NewMax) + " - " + str(NewMin) + ") * ((!" + str(FieldToNormalize) + "! - " +\
           str(OldMin) + ") / (" + str(OldMax) + " - " + str(OldMin) + ")))"
arcpy.CalculateField_management(InputPolygonFeatureClass, "norm", Expr, "PYTHON_9.3", "")
